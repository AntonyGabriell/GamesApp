...
class ActivityScreen2 : AppCompatActivity() {
    ...
    private fun seekBar(){
        ...
        seekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener{
            ...
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, p2: Boolean) {
                ...
                if (progress > 24 && progress < 33){

                    level.setText("Noob")
                    user.nivel = NiveisEnum.Noob
                }
                else if (progress > 33 && progress < 52){

                    level.setText("Basico")
                    user.nivel = NiveisEnum.Basic
                }
                else if (progress > 52 && progress < 62) {

                    level.setText("Casual")
                    user.nivel = NiveisEnum.Casual
                }
                else if (progress > 62 && progress < 72) {

                    level.setText("Amador")
                    user.nivel = NiveisEnum.Amador
                }
                else if (progress > 72 && progress < 82) {

                    level.setText("Pro")
                    user.nivel = NiveisEnum.Pro
                }
                else if (progress > 82 && progress < 92) {

                    level.setText("Tryhard")
                    user.nivel = NiveisEnum.Tryhard
                }
                else if (progress > 92 && progress < 100) {

                    level.setText("Champion")
                    user.nivel = NiveisEnum.Champion
                }
                else  {
